...
// Never display PHP warnings/notices in API responses (they break JSON)
@ini_set('display_errors', '0');
@ini_set('display_startup_errors', '0');
// If needed, errors can be logged by PHP settings/log file
...
// Lazy-load Razorpay only when needed so missing SDK doesn't fatal-error other actions
function getRazorpayApiInstance() {
    $autoloadPath = __DIR__ . '/../../vendor/autoload.php';
    if (!file_exists($autoloadPath)) {
        throw new Exception('Payment gateway not configured (autoload missing).');
    }
    require_once $autoloadPath;
    if (!class_exists('Razorpay\\Api\\Api')) {
        throw new Exception('Payment gateway SDK not installed.');
    }
...
function placeOrder($input, $user_id, $db_connection)
{
    try {
        ...
        foreach ($products as $p) {
            ...
            // Preserve any provided variant metadata (no-op if absent)
