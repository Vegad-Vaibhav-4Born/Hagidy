...
if (!empty($editing_product_id)) {
    ...
    $current_product_session_keys = [
        'pending_product_images_' . $editing_product_id,
        'pending_product_videos_' . $editing_product_id,
        'pending_product_video_' . $editing_product_id,
        'pending_product_path_' . $editing_product_id,
        'pending_product_payload_' . $editing_product_id,
        'existing_specifications_' . $editing_product_id
    ];
    ...
    // Clear session data for other product IDs
    foreach ($_SESSION as $key => $value) {
        if (preg_match('/^pending_product_(images|videos|video|path)_(\d+)$/', $key, $matches)) {
            $other_product_id = $matches[2];
            if ($other_product_id !== $editing_product_id) {
                unset($_SESSION[$key]);
            }
        }
        if (preg_match('/^existing_specifications_(\d+)$/', $key, $matches)) {
    ...
    $rs_prefill = mysqli_query($con, "SELECT * FROM products WHERE id='" . mysqli_real_escape_string($con, $editing_product_id) . "' AND vendor_id='" . mysqli_real_escape_string($con, $vendor_reg_id) . "' LIMIT 1");
...
$form_data = [];
$session_payload_key = !empty($editing_product_id) ? 'pending_product_payload_' . $editing_product_id : 'pending_product_payload';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'remove_image') {
    ...
    if (!empty($image_path) && !empty($product_id)) {
        ...
        if ($rs && mysqli_num_rows($rs) > 0) {
            ...
            if (is_array($current_images)) {
                ...
                foreach ($current_images as $db_img) {
                    ...
                    if ($normalized_db_img === $normalized_image_path || 
        ...
        // Use product ID-specific session key
        $session_images_key = 'pending_product_images_' . $product_id;
        if (isset($_SESSION[$session_images_key]) && is_array($_SESSION[$session_images_key])) {
            $session_images = $_SESSION[$session_images_key];
...
<body>
    ...
    <script>
        async function onCategoryChange(value) {
            ...
            if (!value) { subSel.innerHTML = '<option value="">Sub Category</option>'; return; }
            try {
                const res = await fetch('?ajax=subcategories&category_id=' + encodeURIComponent(value), { headers: { 'Accept': 'application/json' } });
                const data = await res.json();
                subSel.innerHTML = '<option value="">Sub Category</option>';
                if (data && data.success) {
                    data.subcategories.forEach(sc => {
                        const opt = document.createElement('option');
                        opt.value = sc.id; opt.textContent = sc.name; subSel.appendChild(opt);
                    });
                }
        ...
        document.getElementById('saveNextBtn')?.addEventListener('click', function (e) {
            ...
            const requiredFields = [
                { id: 'Light-blue', name: 'Product Name' },
                { id: 'Light-Category', name: 'Category' },
                { id: 'Light-SubCategory', name: 'Sub Category' },
                { id: 'Light-MRP', name: 'MRP' },
                { id: 'Light-Selling', name: 'Selling Price' },
                { id: 'Light-Description', name: 'Description' },
                { id: 'Light-GST', name: 'GST' },
                { id: 'Light-HSN', name: 'HSN ID' },
                { id: 'Light-Manufacturer', name: 'Manufacturer Details' },
                { id: 'Light-Packer', name: 'Packer Details' },
                { id: 'Light-SKU', name: 'SKU ID' },
                { id: 'Light-Inventory', name: 'Inventory' }
            ...
            if (mrpInput && sellingInput && discountInput) {
                ...
                if (mrp > 0 && selling > 0) {
                    if (selling > mrp) {
                        ...
                        const fb = (function(){ let el = sellingInput.parentElement?.querySelector('.invalid-feedback'); if(!el){ el = document.createElement('div'); el.className='invalid-feedback'; sellingInput.parentElement.appendChild(el);} return el; })();
        ...
        document.getElementById('product_video_display')?.addEventListener('change', function (e) {
            ...
            if (!preview) { preview = document.createElement('div'); preview.className = 'video-preview mt-2'; videoContainer.appendChild(preview); }
